...
class Item {
  ...
  constructor(
    ...
    this._capitalismService.subscribeItemConfig(
      itemConfig.uuid,
      this.updateConfig
    )
  ...
  private initializeManager = async () => {
    ...
    this._itemConfig = this._capitalismService.getItemConfigByUuid(
      this._itemConfig.uuid
    )
    const now = new Date()
    ...
    if (this._currentProducing) {
      ...
      const underProducing =
        this._itemConfig.productionStartAt.getTime() +
          this._itemConfig.productionTime >
        now.getTime()
      if (underProducing) {
        initialProgressTime =
          now.getTime() - this._itemConfig.productionStartAt.getTime()
      }
    }
  ...
  private _produceItem = async (progressTime = 0): Promise<void> =>
    new Promise((resolve) => {
  ...
  private renderRevenueAndUpgradeCost() {
    this._upgradeCostElem.innerText = `upgrade: ${this._upgradeCost.toFixed(
      DISPLAY_DECIMAL
    )}`
